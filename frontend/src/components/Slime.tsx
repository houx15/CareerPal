...
function Breathe({ children, intensity = 1, dur = 3.2 }: { children: React.ReactNode; intensity?: number; dur?: number }) {
...
function SlimeListening() {
  return (
    <Breathe intensity={1.2} dur={2.8}>
      <path
        fill={SLIME_FILL}
        d="M155 363C95 312 97 218 161 149C226 80 309 87 345 133C376 173 350 218 383 271C417 326 374 395 286 404C232 410 190 393 155 363Z"
      />
  ...
  return (
    <Breathe intensity={0.9} dur={2.6}>
      <path fill={SLIME_FILL} d="M256 101C342 101 410 169 410 256C410 343 342 411 256 411C170 411 102 343 102 256C102 169 170 101 256 101Z">
        <animate
          attributeName="d"
          values="M256 101C342 101 410 169 410 256C410 343 342 411 256 411C170 411 102 343 102 256C102 169 170 101 256 101Z;M256 99C344 101 412 171 410 258C408 345 344 413 256 413C168 413 100 343 102 254C104 167 168 97 256 99Z;M256 101C342 101 410 169 410 256C410 343 342 411 256 411C170 411 102 343 102 256C102 169 170 101 256 101Z"
          dur="3.6s"
          repeatCount="indefinite"
        />
      </path>
...
function SlimeAnswering() {
  return (
    <Breathe intensity={1.4} dur={2.4}>
      <path
        fill={SLIME_FILL}
        d="M121 247C124 204 159 188 188 165C218 142 221 99 273 99C318 99 333 139 352 165C371 190 408 202 410 255C414 334 352 403 262 405C180 407 115 343 121 247Z"
      />
...
export function Slime({ size = 80, state = "listening" }: { size?: number; state?: SlimeState }) {
  const Body = state === "thinking" ? SlimeThinking : state === "speaking" || state === "answering" ? SlimeAnswering : SlimeListening;

  return (
    <svg
      role="img"
      aria-label={`CareerPal companion ${state}`}
      width={size}
      height={size}
      viewBox="0 0 512 512"
